...
speed = {'bike': 8, 'car': 7.5, 'bus': 7, 'truck': 7, 'pedestrian': 5}  # Vehicle Average speeds
...
logging.basicConfig(filename='LogFile', filemode='w',format='%(asctime)s %(levelname)-8s %(message)s',level=logging.INFO,datefmt='%Y-%m-%d %H:%M:%S')
...
class Pedestrians(pygame.sprite.Sprite):
    def __init__(self, ped_direction_num, ped_direction):
        ...
        '''if ped_direction == 'NB':
            ...
            pedestrians[ped_direction][0][self.item].pedy += temp
        elif ped_direction == 'SB':
            temp = self.image.get_rect().height + ped_stopGap
            pedestrians[ped_direction][0][self.item].pedy -= temp
...
def generate_pedestrians():
    while True:
        ...
        if temp <10:
            ...
            ped_directionNum = 1
        Pedestrians(ped_directionNum, ped_directionNumbers[ped_directionNum])
        time.sleep(1)
...
class Main:
    ...
    thread4 = threading.Thread(name="generatePedestrians", target=generate_pedestrians, args=())
    thread4.daemon = True
    thread4.start()
